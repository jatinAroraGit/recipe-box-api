...
let ListItem = require('../routing/listItem');
...
// A shopping list is created for a user on account creating by default.
...
exports.updateShoppingList = async function (req, res) {
  let data = req.body;
  let newItems = req.body.listItems;
  console.log(newItems);
  let newItemsSplit = newItems.split('~');
  ...
  if (shoppingList.listItems && shoppingList.listItems != '') {
    orgItems = shoppingList.listItems.split(',');
    for (var i = 0; i < orgItems.length; i++) {
      let oneOrgItem = orgItems[i].split('~');
      if (oneOrgItem[0] == newItemsSplit[0] && oneOrgItem[2] == newItemsSplit[2]) {
        finalITem = oneOrgItem[0] + '~' + (parseFloat(oneOrgItem[1]) + parseFloat(newItemsSplit[1])) + '~' + oneOrgItem[2];
        orgItems.splice(i, 1);
        sameItem = true;
        break;
      }
    ...
    if (!sameItem) {
      newListItems = shoppingList.listItems + ',' + req.body.listItems;
    }
    else if (sameItem) {
      orgItems.push(finalITem);
      newListItems = orgItems.join(',');
    }
  }
  else {
  ...
  await ShoppingList.update({ listItems: newListItems }, {
    where: {
      userId: req.body.userId
    }
...
exports.getShoppingList = async function (req, res) {
  let data = req.body;
  let finalResult = {
    items: []
  }
  // let recipe = req.body.recipe;
  let shoppingListDetail = await ShoppingList.findOne({
    where: {
      userId: req.body.userId
    }
  });
  if (shoppingListDetail.listItems && shoppingListDetail.listItems != '') {
    let items = shoppingListDetail.listItems.split(',');
    for (var i = 0; i < items.length; i++) {
      let eachItem = items[i];
      let oneItem = eachItem.split('~');
      let listItem = new ListItem(oneItem[0], oneItem[1], oneItem[2]);
      finalResult.items.push(listItem);
    }
  }
...
exports.updateShoppingListOneItem = async function (req, res) {
  let data = req.body;
